...
let people = jsonArray.map((item) => {
  return "alterego" in item && "ciudad" in item && "publicado" in item
    ? new Heroe(
        item.id,
        item.nombre,
        item.apellido,
...
const addCheckboxs = () => {
  ...
  const selectedInputs =
    selectedValue === "persona"
      ? [...inputsDefault, ...inputsHero, ...inputsVillain]
      : selectedValue === "heroe"
      ? [...inputsDefault, ...inputsHero]
      : [...inputsDefault, ...inputsVillain];
...
const createRow = (item) => {
  ...
  const selectedInputs = [...inputsDefault, ...inputsHero, ...inputsVillain];
...
const addInputByDefault = (initialValues) => {
  ...
  if (initialValues !== undefined) {
    ...
    selectTypeOfPerson.value =
      initialValues instanceof Heroe ? "heroe" : "villano";
...
const addInputByTypePerson = (initialValues) => {
  ...
  const selectedInputs = typeField.value === "heroe" ? inputsHero : inputsVillain;
...
const addPerson = (formData) => {
  ...
  const personObject =
    "alterego" in item && "ciudad" in item && "publicado" in item
      ? new Heroe(
          item.id,
          item.nombre,
          item.apellido,
